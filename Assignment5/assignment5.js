...
function stringFromArrays(arr1, arr2) {
	...
	return String(arr1.shift()) + String(arr2.shift()) + stringFromArrays(arr1, arr2)
}
...
function computeMaxArr(f1, f2) {
	...
	return function (arr)
	{
		...
		let x = arr.shift()
		let maxVal = Math.max(f1(x), f2(x))

		return [maxVal].concat(computeMaxArr(f1, f2)(arr))
	}
}
...
function makeClosure() {
	...
	return [addFunction, incrementCounter]
}
